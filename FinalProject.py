...
# Bagging
bag_weather = RF(max_features = X_train.shape[1], n_estimators = 500, random_state = 0) # Building 500 trees result in a similar MSE (0.04062) when compared to a default of building 100 trees (0.04278)
...
print(f"Bagging Classification Error: {classification_error}")
...
# Random Forest
RF_weather = RF(max_features = 4, n_estimators = 500, random_state = 0) 
RF_weather.fit(X_train, y_train)
# ax = subplots(figsize=(8,8))[1] 
y_hat_bag = RF_weather.predict(X_test) 
# ax.scatter(y_hat_bag, y_test) 
classification_error = np.mean(y_hat_bag != y_test)
print(f"Random Forest Classification Error: {classification_error}")
